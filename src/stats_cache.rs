...
impl StatsCache {
    pub fn new() -> Self {
        ...
        let cache = Self { db_path, copilot_dir };
        cache.init_db();
        cache
    }
    ...
    fn init_db(&self) {
        let conn = match Connection::open(&self.db_path) {
            Ok(c) => c,
            Err(e) => { error!("Failed to open stats cache: {}", e); return; }
        };
        conn.execute_batch("
    ...
    fn sync_session_store(&self, conn: &Connection) {
        ...
        let source = match Connection::open_with_flags(
            ...
            Ok(c) => c,
            Err(e) => { error!("Cannot read session-store.db: {}", e); return; }
        };
        ...
        if let Ok(mut stmt) = source.prepare(
            ...
            stmt.query_map([], |r| {
                ...
                Ok(())
            }).ok().map(|rows| rows.for_each(drop));
        }

        // Sync file counts
        let total_files: i64 = source.query_row("SELECT COUNT(*) FROM session_files", [], |r| r.get(0)).unwrap_or(0);
        let unique_files: i64 = source.query_row("SELECT COUNT(DISTINCT file_path) FROM session_files", [], |r| r.get(0)).unwrap_or(0);
        conn.execute("DELETE FROM session_files_count", []).ok();
        conn.execute("INSERT INTO session_files_count (total, unique_files) VALUES (?1, ?2)", rusqlite::params![total_files, unique_files]).ok();
        ...
        if let Ok(mut stmt) = source.prepare(
            ...
            stmt.query_map([], |r| {
                conn.execute(
                    ...
                    rusqlite::params![r.get::<_, String>(0)?, r.get::<_, i64>(1)?, r.get::<_, i64>(2)?],
                ).ok();
                Ok(())
            }).ok().map(|rows| rows.for_each(drop));
        }
    ...
    pub fn refresh(&self) {
        ...
        if !session_state.exists() { return; }

        let conn = match Connection::open(&self.db_path) {
            Ok(c) => c,
            Err(e) => { error!("Stats cache open failed: {}", e); return; }
        };
        ...
        for events_file in &entries {
            ...
            if last_offset >= file_size { continue; }
            ...
            if (last_offset as usize) >= bytes.len() { continue; }
    ...
    pub fn get_stats(&self) -> CachedStats {
        ...
        let total_turns: i64 = conn
            .query_row("SELECT COALESCE(SUM(turn_count), 0) FROM sessions", [], |r| r.get(0))
            .unwrap_or(0);
